...
async function writeNewEstimate (titleObject) {
   console.log('run')
    ...
    await workbook.xlsx.readFile('../tv_production_template.xlsx')
   ...
   for (const script in titleObject) {
      ...
      worksheet.getCell(`H${currentRow}`).value = `${length}`
      currentRow++

      if (double === true) {
         worksheet.getCell(`B${currentRow}`).value = secondCode 
         worksheet.getCell(`C${currentRow}`).value = title
         worksheet.getCell(`H${currentRow}`).value = `:${length === ":30" ? ':27/03' : length === ':15' ? '12/03' : 'undefined' }`
         currentRow++
      }
